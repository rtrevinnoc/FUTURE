...
$(function() {
  ...
  var submit_form = function(e) {
    ...
    $.getJSON($SCRIPT_ROOT + '/_answer', {
      query: searchbar.val()
        ...
        links_button.animate({ color: "#9e3434" }, "fast");
        summary_button.animate({ color: "#1b1a1a" }, "fast");
        maps_button.animate({ color: "#1b1a1a" }, "fast");
        images_button.animate({ color: "#1b1a1a" }, "fast");
        if (searchbar.val() != response["corrected"]) {
          searchbar.val(response["corrected"])
          searchbar.animate({ backgroundColor: "#9e3434" }, "fast");
          searchbar.animate({ backgroundColor: "#2b2b2b" }, "fast");
        }
  ...
  summary_button.click(function(e){
    summary_button.animate({ color: "#9e3434" }, "fast");
    links_button.animate({ color: "#1b1a1a" }, "fast");
    maps_button.animate({ color: "#1b1a1a" }, "fast");
    images_button.animate({ color: "#1b1a1a" }, "fast");
    section = "summary"
    changeSection()
  ...
  images_button.click(function(e){
    images_button.animate({ color: "#9e3434" }, "fast");
    links_button.animate({ color: "#1b1a1a" }, "fast");
    summary_button.animate({ color: "#1b1a1a" }, "fast");
    maps_button.animate({ color: "#1b1a1a" }, "fast");
    section = "images"
  ...
  maps_button.click(function(e){
    maps_button.animate({ color: "#9e3434" }, "fast");
    links_button.animate({ color: "#1b1a1a" }, "fast");
    summary_button.animate({ color: "#1b1a1a" }, "fast");
    images_button.animate({ color: "#1b1a1a" }, "fast");
    section = "maps"
  ...
  var changeSection = function() {
    if (section == "summary") {
      $("body").animate({ backgroundColor: "#505050" }, "slow");
      summary.animate({ color: "#BABABA" }, "slow");
    } else if (section == "links") {
      $("body").animate({ backgroundColor: "#EEEEEE" }, "slow");
      links.animate({ color: "#1C1C1C" }, "slow");
    } else if (section == "maps") {
      $("body").animate({ backgroundColor: "#EEEEEE" }, "slow");
      maps.animate({ color: "#1C1C1C" }, "slow");
    } else if (section == "images") {
      $("body").animate({ backgroundColor: "#EEEEEE" }, "slow");
      images.animate({ color: "#1C1C1C" }, "slow");
    } else {
      $("body").animate({ backgroundColor: "#EEEEEE" }, "slow");
    } 
  }
  ...
  var toggle_sidebar = function(e) {
    sidebar_menu.animate({width: 'toggle'});
    switch ($('#sidebar_show_icon').attr("class")){
        case 'ion-chevron-left':
            ...
            $('#particles-js').animate({width: ($(window).width() - ($(window).width()/4.1))}, function() {
              $("#particles-js").animate({ opacity: 0.5 });
              $("body").animate({ backgroundColor: "#111111" }, "slow");
        ...
        case "ion-chevron-right":
            ...
            $('#particles-js').animate({width: $(window).width()}, function() {
              changeSection();
              $("#particles-js").animate({ opacity: 1 });
              pJSDom[0].pJS.particles.opacity.value = 0
